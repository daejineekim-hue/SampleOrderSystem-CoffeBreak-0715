...
- **MVC 계층 분리** (PoC1 ConsoleMVC 참조), 전부 `src/` 아래 폴더로 구분 (프로젝트 분리 아님):
  ...
  - `src/repository/`: JSON CRUD + "검증 없이 그냥 반영"하는 저장 연산(예: `updateStock`,
    `updateStatus`)만 담당. 어떤 전이가 유효한지(승인/거절/출고 가능 여부 등)는
    repository의 책임이 아니다 (PoC2 DataPersistence 참조). model은 repository를
    몰라야 함(의존 방향: controller → service → repository → model).
  - `src/service/`: 여러 repository/production을 오케스트레이션하는 유스케이스별
    전이 검증 로직 (예: `OrderLifecycleService`의 승인/거절/출고 — 어떤 상태에서만
    허용되는지 등의 규칙이 여기 있다). `src/production/`처럼 스스로 판단해 완료
    처리를 하는 도메인 로직과는 성격이 다르다 — service는 사용자가 트리거하는
    전이만 다루고, 결과 콜백 없이 단방향으로만 의존한다 (docs/design_refact.md 참고).
  - `src/production/`: 생산 라인(FIFO 큐, lazy 완료 판정) 도메인 로직. 완료 시점의
    재고/상태 반영(Formula #3)은 `ProductionLine`이 `OrderRepository`/
    `SampleRepository`에 직접 반영하며, 다른 서비스로 콜백하지 않는다(순환 의존 방지).
  - `src/generator/`: 더미(demo) 데이터 생성 로직 (PoC4 DummyDataGenerator 참조).
  - `src/test/`: GoogleTest/GoogleMock 테스트 (Test 구성에서만 컴파일됨, 아래 참고).
- **콘솔 서브메뉴 컨트롤러**: 메뉴 루프가 있는 기능 컨트롤러(시료 관리/시료 주문/
  주문 승인/출고 처리)는 `src/controller/ConsoleSubmenuController`를 상속해
  `showPrompt()`/`handle(choice)`만 구현한다 — `while(true)`/입력 파싱 골격을
  중복 작성하지 않는다. 1회성 화면(모니터링/생산라인 조회/더미 데이터)은 대상이 아니다.
...
- **단일 프로젝트 + 3가지 Configuration** (`src/SampleOrderSystem.vcxproj` 하나뿐, Core/App/Tests를
  ...
  - `src/model|view|controller|repository|service|production|generator|json/`의 소스는 세 구성 모두에서
