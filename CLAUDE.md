...
- 단위테스트는 GoogleTest/GoogleMock, NuGet 패키지 `gmock` 1.11.0으로 제공받는다
  (`test/packages.config`). vcpkg는 이 환경에 설치되어 있지 않아 사용하지 않는다.
  패키지 파일 일체(`packages/`)는 재현성을 위해 git에 커밋한다 (gitignore하지 않음) —
  이 환경에서 `nuget.exe` CLI restore가 보장되지 않기 때문.
- `gmock` 패키지는 `test/Tests.vcxproj`에만 참조한다 (Core/App은 테스트 프레임워크에
  의존하지 않는다). `gmock.targets`가 `gtest-all.cc`/`gmock-all.cc`를 자동으로 컴파일에
  추가하며, 진입점은 패키지가 제공하는 `gmock_main.cc`를 사용한다.
- CMake/Make는 사용하지 않는다 (VS 프로젝트 파일 + MSBuild/vstest로 빌드·실행).
  `build.ps1`/`test.ps1`로 커맨드라인 빌드·테스트 가능.
