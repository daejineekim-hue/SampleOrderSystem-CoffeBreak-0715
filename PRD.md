...
- **Order(주문)**: orderId, sampleId, customerName, quantity, status, createdAt.
  ...
  (PRODUCING 상태일 때의 productionStartedAt/producedTotal/productionTimeMin은
  `Order`가 아니라 `ProductionLine`이 보관한다 — [docs/adr/0001](docs/adr/0001-production-fields-live-in-productionline-not-order.md) 참고)
...
`SampleOrderSystem-CoffeBreak-0715`)에 해당하며, PoC 4종(ConsoleMVC/DataPersistence/
